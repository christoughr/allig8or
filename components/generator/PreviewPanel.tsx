...
import { openGeneratedFile } from '@/lib/openFile';
...
> = {
  ...
  presentation: { Icon: Presentation,  title: 'Presentation ready', hint: 'Open in browser or download .pptx.',   ext: 'pptx' },
  spreadsheet:  { Icon: Sheet,         title: 'Spreadsheet ready',  hint: 'Open in browser or download .xlsx.',   ext: 'xlsx' },
  document:     { Icon: FileText,      title: 'Document ready',     hint: 'Open in browser or download .docx.',   ext: 'docx' },
...
function PreviewBar({
  ...
  const canOpen = Boolean(html || fileUrl);
      ...
      <div className="flex flex-1 sm:flex-none items-center justify-end gap-1.5">
        ...
        {canOpen && (
          <button
            type="button"
            onClick={openInTab}
            title="Open in a new tab or Office viewer"
            aria-label="Open"
...
export default function PreviewPanel({
  ...
  if (preview.type === 'file' && preview.fileUrl) {
    ...
    const fileSizeNote = ext === 'PPTX'
      ? 'Opens in PowerPoint, Google Slides, or Office online'
      : ext === 'XLSX'
        ? 'Opens in Excel, Google Sheets, or Office online'
        : 'Opens in Word, Google Docs, or Office online';

    const handleOpen = () => {
      openGeneratedFile(preview.fileUrl!, preview.fileName);
    };

    return (
      <div className="flex flex-1 flex-col overflow-hidden bg-[#070b09] md:min-h-0">
        <PreviewBar
          device={device}
          setDevice={setDevice}
          fileUrl={preview.fileUrl}
        ...
        <div className="flex flex-1 items-center justify-center p-8">
          <div className="flex w-full max-w-sm flex-col items-center gap-6 rounded-2xl border border-white/8 bg-zinc-900/60 p-10 text-center">
            ...
            <div>
              <p className="font-semibold text-white">{preview.fileName ?? `file.${meta.ext}`}</p>
            ...
            <div className="flex w-full flex-col gap-2">
              ...
              <a
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 px-6 py-2.5 text-sm text-zinc-300 transition hover:border-white/20 hover:text-white"
              >
